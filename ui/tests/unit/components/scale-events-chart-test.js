...
module('Unit | Component | scale-events-chart', function (hooks) {
  ...
  test('the current date is appended as a datum for the line chart to render', function (assert) {
    ...
    assert.strictEqual(chart.data.length, events.length + 1);
    ...
    assert.strictEqual(+appendedDatum.time, +this.refTime);
  });
  ...
  test('if the earliest annotation is outside the domain of the events, the earliest annotation time is added as a datum for the line chart to render', function (assert) {
    ...
    assert.strictEqual(chart.data.length, annotationOutside.length + 1);
    assert.deepEqual(
    ...
    assert.strictEqual(appendedDatum.count, annotationOutside[1].count);
    assert.strictEqual(+appendedDatum.time, +annotationOutside[0].time);

    chart.args.events = annotationInside;

    assert.strictEqual(chart.data.length, annotationOutside.length);
    assert.deepEqual(
