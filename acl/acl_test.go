...
func TestAllowSentinelOperation(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "policy write allows sentinel-create",
			policy:    `sentinel { policy = "write" }`,
			operation: SentinelCapabilityCreate,
			expect:    true,
		},
		{
			name:      "policy write allows sentinel-update",
			policy:    `sentinel { policy = "write" }`,
			operation: SentinelCapabilityUpdate,
			expect:    true,
		},
		{
		...
		{
			name:      "policy read denies sentinel-create",
			policy:    `sentinel { policy = "read" }`,
			operation: SentinelCapabilityCreate,
			expect:    false,
		},
		{
			name:      "policy read denies sentinel-update",
			policy:    `sentinel { policy = "read" }`,
			operation: SentinelCapabilityUpdate,
			expect:    false,
		},
		...
		{
			name: "capability sentinel-create allows sentinel-create over read policy",
			policy: `sentinel { policy = "read"
                                capabilities = ["sentinel-create"] }`,
			operation: SentinelCapabilityCreate,
			expect:    true,
		},
		{
		...
		{
			name:      "multiple capabilities allow respective operations",
			policy:    `sentinel { capabilities = ["sentinel-create", "sentinel-update"] }`,
			operation: SentinelCapabilityCreate,
			expect:    true,
		},
		{
	...
	t.Run("management token allows all operations", func(t *testing.T) {
		...
		must.True(t, acl.AllowSentinelOperation(SentinelCapabilityCreate))
		must.True(t, acl.AllowSentinelOperation(SentinelCapabilityUpdate))
	...
	t.Run("ACLs disabled denies all operations", func(t *testing.T) {
		acl := &ACL{aclsDisabled: true}
		must.False(t, acl.AllowSentinelOperation(SentinelCapabilityRead))
		must.False(t, acl.AllowSentinelOperation(SentinelCapabilityCreate))
		must.False(t, acl.AllowSentinelOperation(SentinelCapabilityUpdate))
